...
use App\Http\Requests\v1\AdultInspectionRequest;
use App\Http\Requests\v1\AdultResumeRequest;
use App\Http\Requests\v1\AssesmentRequest;
use App\Models\ActivityLog;
use App\Models\AdultInspection;
use App\Models\AdultResume;
...
class SubAssesmentController extends Controller
{
    /**
     * listing SubDokumen
     *
     ...
     * @return void
     */
    public function listSubDokumen($visitId, Soap $soap, Assesment $assesment)
    {
        try {
            $subAssesments = SubAssesment::where('assesment_id', $assesment->id)->get();
            $data = [];

            foreach ($subAssesments as $key => $subAssesment) {
                $row['id'] = $subAssesment->id;
                $row['name'] = $subAssesment->template->name;
                $row['visibility'] = $subAssesment->template->visibility;
                if ($subAssesment->template->name == 'Assesment Awal Medis' || $subAssesment->template->name == 'Assesment Umum Khusus Medis') {
                    $row['allow'] = 1;
                } else {
                    $row['allow'] = auth()->user()->id == $assesment->user_id  ? 1 : 0;
                }
                $row['role']  = getRoleFromPermission($subAssesment->template->type);
                $row['type'] = 'assesment';
                $row['created_by'] = $subAssesment->created_by;
                $row['updated_at'] = $subAssesment->updated_at;
                $row['updated_by'] = $subAssesment->updated_by;
                $data[] = $row;
            }

            // if($subAssesment->template->type == 'Umum Dewasa') {
            //     $row['id'] = $visitId;
            //     $row['name'] = 'Hasil Pemeriksaan Rawat Jalan Terintegrasi';
            //     $row['visibility'] = 1;
            //     $row['allow'] = 1;
            //     $row['role']  = getRoleFromPermission('Inspection');
            //     $row['type'] = 'inspection';
            //     $row['created_by'] = $assesment->user_id ? $assesment->user->name : '-';
            //     $row['updated_at'] = AdultInspection::latest()->first() ? AdultInspection::latest()->first()->created_at : '-';
            //     $row['updated_by'] = AdultInspection::latest()->first() ? AdultInspection::latest()->first()->user->name : '-';
            //     $data[] = $row;

            //     $row['id'] = $visitId;
            //     $row['name'] = 'Resume Rawat Jalan';
            //     $row['visibility'] = 1;
            //     $row['allow'] = 1;
            //     $row['role']  = getRoleFromPermission('Resume');
            //     $row['type'] = 'resume';
            //     $row['created_by'] = $assesment->user_id ? $assesment->user->name : '-';
            //     $row['updated_at'] = AdultResume::latest()->first() ? AdultResume::latest()->first()->created_at : '-';
            //     $row['updated_by'] = AdultResume::latest()->first() ? AdultResume::latest()->first()->user->name : '-';
            //     $data[] = $row;
            // }
            ...
            return response()->json([
                'status' => 'failed',
                'message' => 'Failed retrieve sub dokumen.',
            ], 500);
        }
    }

    /**
     * listing attribute yang aktif
     *
     * @param  mixed $visitId
     * @param  mixed $soap
     * @param  mixed $assesment
     * @return void
     */
    public function attribute($visitId, Soap $soap, Assesment $assesment, SubAssesment $subAssesment)
    {
        try {
            $attribute = Attribute::select(
                ...
                ->where('templates.id', $subAssesment->template->id)
            ...
            return response()->json($attribute->groupBy('group_name'));
        } catch (\Throwable $th) {
            //throw $th;
            info($th);
            return response()->json([
                'status' => 'failed',
                'message' => 'Failed retrieve attribute.',
            ], 500);
        }
    }

    public function edit($visitId, Soap $soap, Assesment $assesment, SubAssesment $subAssesment)
    {
        try {
            $data = [];
            $data['type'] = $subAssesment->template->type;
            $data['name'] = $subAssesment->template->name;
            $dataCollec = collect($data);
            return response()->json($dataCollec);
        } catch (\Throwable $th) {
            info($th);
            return response()->json([
                'status' => 'failed',
                'message' => 'Failed retrieve sub assesment.',
            ], 500);
        }
    }

    /**
     * lihat data assesment
     *
     * @param  mixed $visitId
     * @param  mixed $soap
     * @param  mixed $assesment
     * @return void
     */
    public function editAssesment($visitId, Soap $soap, Assesment $assesment, SubAssesment $subAssesment)
    {
        try {
            if (
                auth()->user()->can('lihat deteksi dini kewaspadaan terhadap COVID 19') && $subAssesment->template->type == 'Covid' ||
                auth()->user()->can('lihat assesmen awal keperawatan rawat jalan') && $subAssesment->template->type == 'Umum Dewasa' || auth()->user()->can('lihat assesmen awal keperawatan pasien anak rawat jalan') && $subAssesment->template->type == 'Umum Anak' || auth()->user()->can('lihat assesmen awal medis anak') && $subAssesment->template->type == 'Spesialis Anak' || auth()->user()->can('lihat assesmen awal medis penyakit dalam') && $subAssesment->template->type == 'Spesialis Penyakit Dalam' || auth()->user()->can('lihat assesmen global') && $subAssesment->template->type == 'Global' || auth()->user()->can('lihat assesmen awal medis syaraf') && $subAssesment->template->type == 'Spesialis Syaraf' || auth()->user()->can('lihat assesmen awal medis paru') && $subAssesment->template->type == 'Spesialis Paru'
            ) {
                return response()->json($subAssesment);
            } else {
                return response()->json([
                    'status' => 'unautorized',
                ], 403);
     ...
     * @return void
     */
    public function viewAuthor($visitId, Soap $soap, Assesment $assesment, SubAssesment $subAssesment)
    {
        try {
            $row['id'] = $subAssesment->id;
            $row['name'] = $subAssesment->template->name;
            ...
            return response()->json([
                'status' => 'failed',
                'message' => 'Failed retrieve author.',
            ], 500);
        }
    }

    /**
     * update data assesment
     *
     * @param  mixed $request
     * @param  mixed $visitId
     * @param  mixed $soap
     ...
     * @return void
     */
    public function updateAssesment(AssesmentRequest $request, $visitId, Soap $soap, Assesment $assesment, SubAssesment $subAssesment)
    {
        try {
            ...
            if (
                auth()->user()->can('ubah deteksi dini kewaspadaan terhadap COVID 19') && $subAssesment->template->type == 'Covid'
                || auth()->user()->can('ubah assesmen awal keperawatan rawat jalan') && $subAssesment->template->type == 'Umum Dewasa'
                || auth()->user()->can('ubah assesmen awal keperawatan pasien anak rawat jalan') && $subAssesment->template->type == 'Umum Anak'
                || auth()->user()->can('ubah assesmen awal medis anak') && $subAssesment->template->type == 'Spesialis Anak'
                || auth()->user()->can('ubah assesmen awal medis penyakit dalam') && $subAssesment->template->type == 'Spesialis Penyakit Dalam'
                || auth()->user()->can('ubah assesmen global') && $subAssesment->template->type == 'Global'
                || auth()->user()->can('ubah assesmen awal medis syaraf') && $subAssesment->template->type == 'Spesialis Syaraf'
                || auth()->user()->can('ubah assesmen awal medis paru') && $subAssesment->template->type == 'Spesialis Paru'
            ) {
                $subAssesment->update(collect($request->validated())->except('tanggal_timbul_gejala', 'pernah_mondok', 'terakhir_dirawat_simo', 'fungsional', 'laboratorium')->toArray());

                if ($request->tanggal_timbul_gejala) {
                    $subAssesment->update([
                        'tanggal_timbul_gejala' => Carbon::parse($request->tanggal_timbul_gejala)->setTimezone('Asia/Jakarta')
                    ]);
                }

                if ($request->terakhir_dirawat_simo) {
                    $subAssesment->update([
                        'terakhir_dirawat_simo' => Carbon::parse($request->terakhir_dirawat_simo)->setTimezone('Asia/Jakarta')
                    ]);
                }

                if ($request->kiriman_dari) {
                    $subAssesment->update([
                        'kiriman_dari' => $request->kiriman_dari
                    ]);
                }

                if ($request->nutrisional_dewasa_penurunan_bb) {
                    $subAssesment->update([
                        'nutrisional_dewasa_penurunan_bb_pilihan' => $request->nutrisional_dewasa_penurunan_bb_pilihan,
                        'nutrisional_dewasa_penurunan_bb_total_skor' => $request->nutrisional_dewasa_penurunan_bb_total_skor
                    ]);
                }
                ...
                if ($subAssesment->template_id == 1 || $subAssesment->template_id == 6) {
                    $assesment->update([
                        'status' => 'done'
                    ]);

                    $soap->update(['status' => 'done']);
                }

                if (in_array($subAssesment->template_id, [4, 5, 7, 8])) {
                    IntegrationResult::updateOrCreate([
                        ...
                        "info" => 'Ke ' . $soap->visit_number . ' pada SOAP ' . $soap->soap_number,
                        "assesment_name" => Template::find($subAssesment->template_id)->type,
                        "rencana_tindak_lanjut" => json_encode($subAssesment->rencana_tindak_lanjut),
                ...
                if (in_array($subAssesment->template_id, [2, 3])) {
                    IntegrationResult::updateOrCreate([
                        'sub_assesment_id' => $subAssesment->id,
                    ], [
                        "visit_id" => $visitId,
            ...
            ActivityLog::create([
                ...
                'unique_id' => $subAssesment->id,
                'visit_id' => $visitId,
                'soap_id' => $soap->id,
                'dokumen_id' => $assesment->id,
                'note' => 'Mengubah sub dokumen ' . $subAssesment->template->name . '',
                'type' => 'Assesmen',
                'action' => 'Mengubah'
            ]);
            ...
            return response()->json([
                'status' => 'success',
                'message' => 'Assesment successfully updated.',
            ]);
        } catch (\Throwable $th) {
            //throw $th;
            info($th);
     ...
     * @return void
     */
    public function printPDF($visitId, Soap $soap, Assesment $assesment)
    {
        $header = $this->headerListing();
        $visits = Http::withHeaders(['x-tstamp' => $header['timestamp'], 'x-sign' => $header['sign']])->get(''.env('VISIT_URL').'/trx/kunjungan?kode=' . $visitId);
        $norm = $visits->status() == 200 ? $visits[0]['norm'] : '';
        // $inspections = AdultInspection::where('patient_id', $norm)->get();
        // $resumes = AdultResume::where('patient_id', $norm)->get();
        // $allergy = AllergyHistory::where('patient_id', $norm)->first();
        $resultLab = $this->resultRequestLab();
        $html = '';
        ...
        foreach ($subAssesments as $key => $subAssesment) {
            ...
            if ($subAssesment->template->type == 'Spesialis Anak' || $subAssesment->template->type == 'Spesialis Penyakit Dalam' || $subAssesment->template->type == 'Spesialis Paru' || $subAssesment->template->type == 'Spesialis Syaraf') {
                // info($resultLab);
                $laboratorium = collect();
                if ($subAssesment->laboratorium_id) {
                    $laboratorium = RequestLab::find($subAssesment->laboratorium_id);
                }
            }

            if ($subAssesment->template->type == 'Umum Dewasa') {
                $html = '';
                $view = view('pdf.assesment')->with([
                    'visits' => $visits->status() == 200 ? $visits[0] : [],
                    'attributes' => $attributes->groupBy('group_name'),
                    'assesments' => $subAssesment,
                    'soap' => $soap
                ]);
                $html .= $view->render();

                // $inspection = view('pdf.inspection')->with([
                //     'visits' => $visits->status() == 200 ? $visits[0] : [],
                //     'inspections' => $inspections
                // ]);
                // $html .= $inspection->render();

                // $resume = view('pdf.resume')->with([
                //     'visits' => $visits->status() == 200 ? $visits[0] : [],
                //     'resumes' => $resumes,
                //     'allergy' => $allergy
                // ]);
            ...
            if ($subAssesment->template->type == 'Umum Anak') {
                // $html = '';
                $view = view('pdf.assesment')->with([
                    'visits' => $visits->status() == 200 ? $visits[0] : [],
                    'attributes' => $attributes->groupBy('group_name'),
            ...
            if ($subAssesment->template->type == 'Covid' || $subAssesment->template->type == 'Global') {
                $pdf = Pdf::loadView('pdf.assesment', [
                    'visits' => $visits->status() == 200 ? $visits[0] : [],
                    'attributes' => $attributes->groupBy('group_name'),
                    'assesments' => $subAssesment,
                    'soap' => $soap
            ...
            if ($subAssesment->template->type == 'Spesialis Penyakit Dalam' || $subAssesment->template->type == 'Spesialis Anak' || $subAssesment->template->type == 'Spesialis Syaraf' || $subAssesment->template->type == 'Spesialis Paru') {
                $pdf = Pdf::loadView('pdf.assesment', [
                    'visits' => $visits->status() == 200 ? $visits[0] : [],
                    'attributes' => $attributes->groupBy('group_name'),
                    'assesments' => $subAssesment,
        ...
        return $pdf->download('Assesment-' . $norm . '.pdf');
    }

    /**
     * Print PDF per assesment
     *
     * @return void
     */
    public function printPdfSubAssesment($visitId, Soap $soap, Assesment $assesment, SubAssesment $subAssesment)
    {
        $header = $this->headerListing();
        $visits = Http::withHeaders(['x-tstamp' => $header['timestamp'], 'x-sign' => $header['sign']])->get(''.env('VISIT_URL').'/trx/kunjungan?kode=' . $visitId);
        $norm = $visits->status() == 200 ? $visits[0]['norm'] : '';
    ...
    // /**
    //  * Print PDF hasil pemeriksaan
    //  *
    //  * @return void
    //  */
    // public function printPdfInspection($visitId)
    // {
    //     $header = $this->headerListing();
    //     $visits = Http::withHeaders(['x-tstamp' => $header['timestamp'], 'x-sign' => $header['sign']])->get(''.env('VISIT_URL').'/trx/kunjungan?kode=' . $visitId);
    //     $norm = $visits->status() == 200 ? $visits[0]['norm'] : '';
    ...
    //     return $pdf->download('Hasil Pemeriksaan Rawat Jalan Terintegrasi-'.$norm.'.pdf');
    // }
    ...
    //  * @return void
    //  */
    // public function printPdfResume($visitId)
    // {
    //     $header = $this->headerListing();
    //     $visits = Http::withHeaders(['x-tstamp' => $header['timestamp'], 'x-sign' => $header['sign']])->get(''.env('VISIT_URL').'/trx/kunjungan?kode=' . $visitId);
    //     $norm = $visits->status() == 200 ? $visits[0]['norm'] : '';
    ...
    // /**
    //  * Function untuk menampilkan list hasil pemeriksaan
    //  *
    //  * @param  mixed $request
    //  * @param  mixed $soap
    ...
    //  * @return void
    //  */
    // public function listingInspection(Request $request, $visitId, Soap $soap)
    // {
    //     try {
    //         $header = $this->headerListing();
    //         $visits = Http::withHeaders(['x-tstamp' => $header['timestamp'], 'x-sign' => $header['sign']])->get(''.env('VISIT_URL').'/trx/kunjungan?kode=' . $visitId);
    //         $norm = $visits->status() == 200 ? $visits[0]['norm'] : 1;
    //         $itemPerpage = $request->pagination ? $request->pagination : 10;
    //         $currentPage = $request->get('page', 1);
    //         $inspections = AdultInspection::select(
    //             "adult_inspections.id",
    //             "adult_inspections.user_name",
    //             "adult_inspections.user_role",
    //             "adult_inspections.test_date",
    //             "adult_inspections.test_result",
    //             "adult_inspections.soap",
    //             "adult_inspections.updated_at")
    //         ->where('patient_id', $norm)
    //         ->when($request->filled('search'), function ($query) use ($request) {
    //             $query->where(function ($query) use ($request) {
    //                 $dateCreatedUpdated = searchDate($request->search, 'd/m/Y', 'Y-d-m');
    //                 $dateInspection = searchDate($request->search, 'd/m/Y', 'Y-d-m');
    //                 $query->where('user_name', 'like', '%' . $request->search . '%');
    //                 $query->orWhere('user_role', 'like', '%' . $request->search . '%');
    //                 $query->orWhere('test_result', 'like', '%' . $request->search . '%');
    //                 $query->orWhere('soap', 'like', '%' . $request->search . '%');
    //                 info($request->search);
    //                 if($dateInspection != 'date invalid') {
    //                     $query->orWhere('test_date', 'like', '%' . $dateInspection . '%');
    //                 }
    //                 if($dateCreatedUpdated != 'date invalid') {
    //                     $query->orWhere('created_at', 'like', '%' . $dateCreatedUpdated . '%');
    //                     $query->orWhere('updated_at', 'like', '%' . $dateCreatedUpdated . '%');
    //                 }
    //             });
    //         })
    //         ->orderBy($request->order_by ? $request->order_by : 'created_at', $request->order_dir ? $request->order_dir : 'desc');

    //         $inspectionPaginate = (new CustomPaginator(
    //             $inspections->latest()->clone()->forPage($currentPage, $itemPerpage)->get(),
    //             $inspections->clone()->count(),
    //             $itemPerpage,
    //             $currentPage,
    //         ))
    //             ->appends($request->all())
    //             ->withPath(env('APP_URL').'/soap/adult/inspection/listing/'. $request->patientId);

    //         return response()->json($inspectionPaginate);
    //     } catch (\Throwable $th) {
    //         info($th);
    //         return response()->json([
    //             'status'  => 'failed',
    //             'message' => 'Failed to retrieve adult inspections'
    //         ], 500);
    //     }
    // }

    // /**
    //  * Function untuk menambahkan hasil pemeriksaan
    //  *
    //  * @param  mixed $request
    //  * @param  mixed $soap
    //  * @param  mixed $patientId
    //  * @return void
    //  */
    // public function createInspection(AdultInspectionRequest $request, $visitId, Soap $soap)
    // {
    //     try {
    //         $header = $this->headerListing();
    //         $visits = Http::withHeaders(['x-tstamp' => $header['timestamp'], 'x-sign' => $header['sign']])->get(''.env('VISIT_URL').'/trx/kunjungan?kode=' . $visitId);
    //         $norm = $visits->status() == 200 ? $visits[0]['norm'] : 1;
    //         AdultInspection::create([
    //             'patient_id' => $norm,
    //             'user_id' => auth()->user()->id,
    //             'user_name' => auth()->user()->name,
    //             'user_role' => auth()->user()->roles->first()->name,
    //             'soap' => 'SOAP '.$soap->soap_number.', Umum Dewasa',
    //             'test_date' => Carbon::parse($request->test_date)->setTimezone('Asia/Jakarta')->format('d/m/Y') .' '. Carbon::parse($request->test_time)->setTimezone('Asia/Jakarta')->format('H:i:s'),
    //             'test_result' => $request->test_result
    //         ]);

    //         return response()->json([
    //             'status' => 'success',
    //             'message' => 'Adult inspection sucessfully added.'
    //         ]);
    //     } catch (\Throwable $th) {
    //         info($th);
    //         return response()->json([
    //             'status' => 'failed',
    //             'message' => 'Failed update adult inspection.',
    //         ], 500);
    //     }
    // }

    // /**
    //  * Function untuk melihat detail hasil pemeriksaan
    //  *
    //  * @param  mixed $soap
    //  * @param  mixed $adultInspection
    //  * @return void
    //  */
    // public function editInspection(Soap $soap, AdultInspection $adultInspection)
    // {
    //     try {
    //         $data = collect();
    //         $row['test_date'] = Carbon::parse($adultInspection->test_date);
    //         $row['test_time'] = Carbon::parse($adultInspection->test_date);
    //         $row['user_name'] = $adultInspection->user_name;
    //         $row['user_role'] = $adultInspection->user_role;
    //         $row['test_result'] = $adultInspection->test_result;
    //         $row['soap'] = $adultInspection->soap;
    //         $data = $row;
    ...
    //         return response()->json([
    //             'status' => 'failed',
    //             'message' => 'Failed retrieve adult inspection.',
    //         ], 500);
    //     }
    // }

    // /**
    //  * Function untuk mengubah data hasil pemeriksaan
    //  *
    //  * @param  mixed $request
    //  * @param  mixed $soap
    //  * @param  mixed $adultInspection
    //  * @return void
    //  */
    // public function updateInspection(AdultInspectionRequest $request, Soap $soap, AdultInspection $adultInspection)
    // {
    //     try {
    //         $adultInspection->update([
    //             'test_date' => Carbon::parse($request->test_date)->setTimezone('Asia/Jakarta')->format('d/m/Y') .' '. Carbon::parse($request->test_time)->setTimezone('Asia/Jakarta')->format('H:i:s'),
    //             'test_result' => $request->test_result
    //         ]);
    ...
    //         return response()->json([
    //             'status' => 'success',
    //             'message' => 'Adult inspection sucessfully updated.'
    //         ]);
    //     } catch (\Throwable $th) {
    //         info($th);
    //         return response()->json([
    ...
    // /**
    //  * Function untuk hapus hasil pemeriksaan
    //  *
    //  * @param  mixed $soap
    //  * @param  mixed $adultInspection
    //  * @return void
    //  */
    // public function destroyInspection(Soap $soap, AdultInspection $adultInspection)
    // {
    //     try {
    //         $adultInspection->delete();

    //         return response()->json([
    //             'status' => 'success',
    //             'message' => 'Adult inspection sucessfully deleted.'
    //         ]);
    //     } catch (\Throwable $th) {
    //         info($th);
    //         return response()->json([
    //             'status' => 'failed',
    //             'message' => 'Failed delete adult inspection.',
    //         ], 500);
    //     }
    // }

    // /**
    //  * Function untuk melihat alergi
    //  *
    //  * @param  mixed $soap
    //  * @param  mixed $patientId
    //  * @return void
    //  */
    // public function editAllergy(Soap $soap, $visitId)
    // {
    //     try {
    //         $header = $this->headerListing();
    //         $visits = Http::withHeaders(['x-tstamp' => $header['timestamp'], 'x-sign' => $header['sign']])->get(''.env('VISIT_URL').'/trx/kunjungan?kode=' . $visitId);
    ...
    //         return response()->json([
    //             'status' => 'failed',
    //             'message' => 'Failed retrieve allergy history.',
    //         ], 500);
    //     }
    // }

    // /**
    //  * Function untuk mengubah data alergi
    //  *
    //  * @param  mixed $request
    //  * @param  mixed $soap
    //  * @param  mixed $patientId
    //  * @return void
    //  */
    // public function updateAllergy(Request $request, Soap $soap, $visitId)
    // {
    //     try {
    //         $header = $this->headerListing();
    //         $visits = Http::withHeaders(['x-tstamp' => $header['timestamp'], 'x-sign' => $header['sign']])->get(''.env('VISIT_URL').'/trx/kunjungan?kode=' . $visitId);
    ...
    //         return response()->json([
    //             'status' => 'success',
    //             'message' => 'Allergy history successfully updated.',
    //         ]);
    //     } catch (\Throwable $th) {
    //         info($th);
    //         return response()->json([
    //             'status' => 'failed',
    //             'message' => 'Failed update allergy history.',
    //         ], 500);
    //     }
    // }

    // /**
    //  * Function untuk melihat listing resume pemeriksaan
    //  *
    //  * @param  mixed $request
    //  * @param  mixed $soap
    //  * @param  mixed $patientId
    //  * @return void
    //  */
    // public function listingResume(Request $request, $visitId, Soap $soap)
    // {
    //     try {
    //         $header = $this->headerListing();
    //         $visits = Http::withHeaders(['x-tstamp' => $header['timestamp'], 'x-sign' => $header['sign']])->get(''.env('VISIT_URL').'/trx/kunjungan?kode=' . $visitId);
    ...
    //         $itemPerpage = $request->pagination ? $request->pagination : 10;
    //         $currentPage = $request->get('page', 1);
    //         $inspections = AdultResume::select(
    //             "adult_resumes.id",
    //             "adult_resumes.resume_date",
    //             "adult_resumes.user_name",
    //             "adult_resumes.diagnosis",
    //             "adult_resumes.terapi",
    //             "adult_resumes.riwayat_tindakan",
    //             "adult_resumes.soap",
    //             "adult_resumes.updated_at")
    //         ->where('patient_id', $norm)
    //         ->when($request->filled('search'), function ($query) use ($request) {
    //             $query->where(function ($query) use ($request) {
    //                 $dateCreatedUpdated = searchDate($request->search, 'd/m/Y', 'Y-d-m');
    //                 $dateResume = searchDate($request->search, 'd/m/Y', 'Y-d-m');
    //                 $query->where('user_name', 'like', '%' . $request->search . '%');
    //                 $query->orWhere('diagnosis', 'like', '%' . $request->search . '%');
    //                 $query->orWhere('terapi', 'like', '%' . $request->search . '%');
    //                 $query->orWhere('riwayat_tindakan', 'like', '%' . $request->search . '%');
    //                 $query->orWhere('soap', 'like', '%' . $request->search . '%');
    //                 info($request->search);
    //                 if($dateResume != 'date invalid') {
    //                     $query->orWhere('test_date', 'like', '%' . $dateResume . '%');
    //                 }
    //                 if($dateCreatedUpdated != 'date invalid') {
    //                     $query->orWhere('created_at', 'like', '%' . $dateCreatedUpdated . '%');
    //                     $query->orWhere('updated_at', 'like', '%' . $dateCreatedUpdated . '%');
    ...
    // /**
    //  * Function untuk menambah resume pemeriksaan
    //  *
    //  * @param  mixed $request
    //  * @param  mixed $soap
    //  * @param  mixed $patientId
    //  * @return void
    //  */
    // public function createResume(AdultResumeRequest $request, $visitId, Soap $soap)
    // {
    //     try {
    //         $header = $this->headerListing();
    //         $visits = Http::withHeaders(['x-tstamp' => $header['timestamp'], 'x-sign' => $header['sign']])->get(''.env('VISIT_URL').'/trx/kunjungan?kode=' . $visitId);
    ...
    //             'user_role' => auth()->user()->roles->first()->name,
    //             'soap' => 'SOAP '.$soap->soap_number.', Umum Dewasa',
    //             'resume_date' => Carbon::parse($request->resume_date)->setTimezone('Asia/Jakarta')->format('d/m/Y') .' '. Carbon::parse($request->resume_time)->setTimezone('Asia/Jakarta')->format('H:i:s'),
    //             'diagnosis' => $request->diagnosis,
    //             'terapi' => $request->terapi,
    //             'riwayat_tindakan' => $request->riwayat_tindakan
    //         ]);

    //         return response()->json([
    //             'status' => 'success',
    //             'message' => 'Adult resume sucessfully added.'
    //         ]);
    //     } catch (\Throwable $th) {
    //         info($th);
    //         return response()->json([
    //             'status' => 'failed',
    //             'message' => 'Failed update adult resume.',
    //         ], 500);
    //     }
    // }

    // /**
    //  * Function untuk melihat detail resume pemeriksaan
    //  *
    //  * @param  mixed $soap
    //  * @param  mixed $adultResume
    //  * @return void
    //  */
    // public function editResume(Soap $soap, AdultResume $adultResume)
    // {
    //     try {
    //         $data = collect();
    //         $row['resume_date'] = Carbon::parse($adultResume->resume_date);
    //         $row['resume_time'] = Carbon::parse($adultResume->resume_date);
    //         $row['user_name'] = $adultResume->user_name;
    //         $row['user_role'] = $adultResume->user_role;
    //         $row['diagnosis'] = $adultResume->diagnosis;
    //         $row['terapi'] = $adultResume->terapi;
    //         $row['riwayat_tindakan'] = $adultResume->riwayat_tindakan;
    //         $row['soap'] = $adultResume->soap;
    //         $data = $row;
    ...
    // /**
    //  * Function untuk mengubah data resume pemeriksaan
    //  *
    //  * @param  mixed $request
    //  * @param  mixed $soap
    //  * @param  mixed $adultResume
    //  * @return void
    //  */
    // public function updateResume(AdultResumeRequest $request, Soap $soap, AdultResume $adultResume)
    // {
    //     try {
    //         $adultResume->update([
    //             'resume_date' => Carbon::parse($request->resume_date)->setTimezone('Asia/Jakarta')->format('d/m/Y') .' '. Carbon::parse($request->resume_time)->setTimezone('Asia/Jakarta')->format('H:i:s'),
    ...
    //         return response()->json([
    //             'status' => 'success',
    //             'message' => 'Adult resume sucessfully updated.'
    //         ]);
    //     } catch (\Throwable $th) {
    //         info($th);
    //         return response()->json([
    ...
    // /**
    //  * Function untuk hapus resume pemeriksaan
    //  *
    //  * @param  mixed $soap
    //  * @param  mixed $adultResume
    //  * @return void
    //  */
    // public function destroyResume(Soap $soap, AdultResume $adultResume)
    // {
    //     try {
    //         $adultResume->delete();
    ...
    //         return response()->json([
    //             'status' => 'success',
    //             'message' => 'Adult resume sucessfully deleted.'
    //         ]);
    //     } catch (\Throwable $th) {
    //         info($th);
    //         return response()->json([
    //             'status' => 'failed',
    //             'message' => 'Failed delete adult resume.',
    //         ], 500);
    //     }
    // }
