...
<body style="background-image: url('{{ public_path('assets/images/background/Bg-Assesment.png') }}'); background-repeat: no-repeat; background-position: center;">
            ...
            <tr>
                ...
                <td style="width: 20%; vertical-align: top;">
                    <table style="width: 100%; text-align: right;">
                        ...
                        <tr>
                            <td>Kunjungan ke {{ $rehab->visit_number }}</td>
                        </tr>
                    </table>
                </td>
            </tr>
    ...
    <div class="tbr_body mt-2">
        <table style="width: 100%">
            <tr>
                <th>
                    <td class="tbr_font-bold" align="left" style="background-color: #0D9A89; color: #FFFFFF; height: 30px; padding: 0 20px;">REHAB MEDIK - {{ formatFromTo(str_replace("jam", "", $rehab->created_at), 'd/m/Y H:i:s', 'd F Y') }}</td>
                    <td class="tbr_font-bold" align="right" style="background-color: #0D9A89; color: #FFFFFF; height: 30px; padding: 0 20px;">{{ $rehab->unique_id }}</td>
                </th>
            </tr>
        </table>
        <table class="tbr_table--striped table-striped" style="width: 100%">
            ...
            @foreach ($rehab->actionRehabs as $action)
                <tr>
                    ...
                    <td style="width: 50%; padding: 0 20px">{{ $action->name }}</td>
                    @if ($action->is_add)
                        <td style="width: 50%; padding: 0 20px"><img src="{{ public_path('assets/images/icon/Check-Icon-Green.png') }}" alt="{{ $action->name }}" style="margin-top: 10px;"></td>  
                    @else 
                        <td style="width: 50%; padding: 0 20px"></td>
                    @endif
                </tr>
            @endforeach
        </table>
